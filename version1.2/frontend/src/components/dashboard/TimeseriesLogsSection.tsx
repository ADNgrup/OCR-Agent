...
import { formatDate, typeColor } from "./dashboardUtils";
    ...
    <div className="row-2col row-2col-stretch">
      ...
      <section className="card">
        ...
        {logs.length ? (
              ...
              <tbody>
                {logs.map((log) => (
                  <tr key={log.log_id}>
                    ...
                    <td>{log.entity_name || log.entity_key}</td>
                    <td>{log.metric_label || log.metric || ""}</td>
                    <td>
                      {(log.value ?? "").toString()}
                      {log.unit ? ` ${log.unit}` : ""}
                    </td>
          ...
          <p className="muted">No logs found for selected screen/time range.</p>
        )}
      </section>
    </div>
  );
