...
export default function Nav() {
  ...
  return (
    <nav className="sticky top-0 z-50 border-b  pad-5 border-gray-200 bg-white/85 backdrop-blur">
          ...
          {NAV_ITEMS.map((item) => (
            <li key={item.href}>
              <Link
                href={item.href}
                className={
                  "inline-block rounded-lg m-2 px-3.5 py-1.5 text-[13px] font-semibold no-underline transition-colors " +
                  (pathname === item.href
